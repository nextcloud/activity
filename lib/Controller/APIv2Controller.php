...
class APIv2Controller extends OCSController {
	 ...
	 *
	 * Deliberately ignores `from`/`to`: the histogram is the control the reader
	 * picks a date range *with*, so it has to keep showing the days outside the
	 * current selection. Every other restriction is honoured, so the bars always
	 * count the same activities the stream below them lists.
	 *
	 * @NoAdminRequired
	 *
