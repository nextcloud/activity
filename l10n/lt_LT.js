OC.L10N.register(
    "activity",
    {
    "Activities" : "Veiklos",
    "An error occurred while loading activities" : "Įkeliant veiklas, įvyko klaida",
    "No activity yet" : "Kol kas nėra veiklos",
    "Load more activities" : "Įkelti daugiau veiklų",
    "Copied!" : "Nukopijuota!",
    "Not supported!" : "Nepalaikoma!",
    "Press ⌘-C to copy." : "Norėdami nukopijuoti, paspauskite ⌘-C.",
    "Press Ctrl-C to copy." : "Paspauskite Ctrl-C, kad nukopijuotumėte.",
    "{name} (invisible)" : "{name} (nematomas)",
    "{name} (restricted)" : "{name} (apribotas)",
    "in {path}" : "{path}",
    "This stream will show events like additions, changes & shares" : "Srautas rodys tokius įvykius, kaip pridėjimai, pakeitimai ir bendrinimai",
    "There are no events for this filter" : "Nėra įvykių pasirinktam filtrui",
    "Today" : "Šiandien",
    "Yesterday" : "Vakar",
    "Personal activity feed for %s" : "Asmeninis veiklos kanalas, skirtas %s",
    "Your feed URL is invalid" : "Jūsų kanalo URL yra neteisingas",
    "Your settings have been updated." : "Jūsų nustatymai buvo atnaujinti.",
    "Settings have been updated." : "Nustatymai atnaujinti.",
    "Mail" : "Paštas",
    "Stream" : "Srautas",
    "All activities" : "Visos veiklos",
    "By others" : "Kitų",
    "By you" : "Jūsų",
    "\"remote user\"" : "\"nuotolinis naudotojas\"",
    "Hello %s" : "Sveiki, %s",
    "Hello %s," : "Sveiki, %s,",
    "There was some activity at %s" : "%s buvo veiklos",
    "_and %n more _::_and %n more _" : ["ir dar %n","ir dar %n","ir dar 1%n"],
    "Activity notification" : "Veiklos pranešimai",
    ", " : ",",
    "%s and %s" : "%s ir %s",
    "Activity" : "Veikla",
    "Activity feed" : "Veiklos kanalas",
    "Configure the default activity settings for new users." : "Numatytieji aktyvumo nustatymai naujiems vartotojams.",
    "Choose for which activities you want to get an email notification, and which should show up in the activity app stream." : "Pasirinkti kurioms veikloms norite gauti pašto pranešimus, ir kurios turi būti rodomos aktyvumo programėlės sraute.",
    "List your own actions in the stream" : "Rodyti Jūsų pačių veiklą sraute",
    "Notify about your own actions via email" : "Informuoti apie mano paties veiksmus per el. paštą",
    "You need to set up your email address before you can receive notification emails." : "Jums reikia nurodyti el. pašto adresą, kad gauti pranešimų el. laiškus.",
    "Send emails:" : "Siųsti el. laiškus:",
    "Hourly" : "Kas valandą",
    "Daily" : "Kasdien",
    "Weekly" : "Kas savaitę",
    "Settings" : "Nustatymai",
    "Enable RSS feed" : "Įjungti RSS sklaidos kanalą",
    "No more events to load" : "Nėra daugiau įkeltinų įvykių",
    "Files" : "Failai"
},
"nplurals=3; plural=(n%10==1 && n%100!=11 ? 0 : n%10>=2 && (n%100<10 || n%100>=20) ? 1 : 2);");
