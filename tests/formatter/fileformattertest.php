...
class FileFormatterTest extends TestCase {
	...
	public function dataFindCurrentInfo() {
		return [
			[23, '/test1', null, null, '/test1', false,[
				'path'		=> '/test1',
				'is_dir'	=> false,
				'view'		=> '',
			]],
			[23, '/test1', null, null, '/test1', true,[
				'path'		=> '/test1',
				'is_dir'	=> true,
				'view'		=> '',
			]],
			[23, '/test1', null, '/files/test3', '/files/test3', false,[
				'path'		=> '/test3',
				'is_dir'	=> false,
				'view'		=> 'trashbin',
			]],
			[23, '/test1', null, '/files/test3', '/files/test3', true,[
				'path'		=> '/test3',
				'is_dir'	=> true,
				'view'		=> 'trashbin',
			]],
			[23, '/test1', '/test3', null, '/test3', false,[
				'path'		=> '/test3',
				'is_dir'	=> false,
				'view'		=> '',
			]],
			[23, '/test1', '/test3', null, '/test3', true,[
				'path'		=> '/test3',
				'is_dir'	=> true,
				'view'		=> '',
			]],
	...
	/**
	 * @dataProvider dataFindCurrentInfo
	 *
	 * @param int $fileId
	...
	public function te1stFindCurrentInfo($fileId, $filename, $path, $pathTrash, $isDirPath, $isDir, $expected) {
		$this->rootView->expects($this->at(0))
			...
			->willReturn($path);
		if ($path === null) {
			$this->rootView->expects($this->at(2))
				->method('getPath')
				->with($fileId)
		...
		$formatter = $this->getFormatter();
		$this->assertSame($expected, $this->invokePrivate($formatter, 'findCurrentInfo', [$fileId, $filename]));
	}
