...
const store = new Vuex.Store({
	...
	state: {
		...
		activityGroups: loadState('activity', 'activityGroups'),
	...
	getters: {
		...
		checkedActivities(state) {
			return Object.values(state.activityGroups)
				...
				.map(([activityKey, activity]) => activity.methods.map(method => ({ activityKey, method, activity }))) // [[{activityKey, method, activity}, ...], ...]
